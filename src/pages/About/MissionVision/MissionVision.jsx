...
const MissionVision = ({ about, loading }) => {
  ...
  useEffect(() => {
    if (!isVisible || !about?.mission || missionTyped.length === about.mission.length) return;
  ...
  useEffect(() => {
    if (!isVisible || !about?.vission || visionTyped.length === about.vission.length) return;
  ...
  return (
    <section 
      ref={sectionRef}
      className="relative overflow-hidden bg-gradient-to-b from-black via-gray-900 to-black py-24"
    >
      ...
      <div className="container relative mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 gap-8 relative">
          ...
          <div 
            ref={missionRef}
            className={`
              relative group backdrop-blur-sm bg-gradient-to-br from-gray-900/80 to-black/80 
              border border-gray-800 rounded-3xl p-10 overflow-hidden
          ...
          >
            {/* Card glow effect */}
            {/* <div className="absolute -inset-1 bg-gradient-to-r from-primary/20 via-transparent to-accent/20 rounded-3xl blur opacity-0 group-hover:opacity-100 transition duration-500"></div> */}
            ...
            <div className="relative z-10">
              ...
              <p className="text-gray-300 leading-relaxed text-lg font-light min-h-[120px]">
                {missionTyped}
                {missionTyped.length < about.mission.length && (
                  <span className="inline-block w-1 h-5 ml-1 bg-primary animate-pulse"></span>
                )}
              </p>
              ...
              <div className="mt-6 flex items-center gap-2">
                <div className="h-1 flex-1 bg-gray-800 rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${(missionTyped.length / about.mission.length) * 100}%` }}
                  ></div>
                </div>
                <span className="text-xs text-gray-400 font-mono">
                  {Math.round((missionTyped.length / about.mission.length) * 100)}%
                </span>
              </div>
            </div>
          </div>
          ...
          >
            {/* Card glow effect */}
            {/* <div className="absolute -inset-1 bg-gradient-to-r from-secondary/20 via-transparent to-primary/20 rounded-3xl blur opacity-0 group-hover:opacity-100 transition duration-500"></div> */}
            ...
            <div className="relative z-10">
              ...
              <p className="text-gray-300 leading-relaxed text-lg font-light min-h-[120px]">
                ...
                {visionTyped.length < about.vission.length && (
                  <span className="inline-block w-1 h-5 ml-1 bg-secondary animate-pulse"></span>
                )}
              </p>
              ...
              <div className="mt-6 flex items-center gap-2">
                <div className="h-1 flex-1 bg-gray-800 rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${(visionTyped.length / about.vission.length) * 100}%` }}
                  ></div>
                </div>
                <span className="text-xs text-gray-400 font-mono">
                  {Math.round((visionTyped.length / about.vission.length) * 100)}%
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="mt-20 flex justify-center gap-4">
          ...
          <div className="w-3 h-3 bg-accent rounded-full animate-bounce" style={{ animationDelay: '0.3s' }}></div>
        </div>
      </div>
    </section>
  );
